...
if __name__ == '__main__':
    ...
    if not os.path.exists(r'..\\csv_results_table\\' + 'DS_' + vector_size[0] + '.csv'):
        header = ['Model', 'Embedding', 'Classifier', 'Accuracy', 'Precision', 'Recall', 'F1']
        with open('..\\csv_results_table\\' + 'DS_' + vector_size[0] + '.csv', 'w', encoding='UTF8') as f:
            writer = csv.writer(f)
            # write the header
    ...
    for ep in epochs:
        for emb in embedding_type:
            ...
            comparison_df = pd.read_csv('..\\csv_results_table\\' + 'DS_' + vector_size[0] + '.csv')
            ...
            for name, model in zip(names, classifiers):
                ...
                # if name == "Threshold Classifier":
                #     model = model.fit(X_train, y_train)
                #     yhat = model.predict_proba(X_test)
                #     yhat = yhat[:, 1]
                #     thresholds = arange(0, 1, 0.0001)
                #     scores = [accuracy_score(y_test, to_labels(yhat, t)) for t in thresholds]
                #     ix = argmax(scores)
            ...
            comparison_df.to_csv(
                '..\\csv_results_table\\' + 'DS_' + vector_size[0] + '.csv',
                index=False)
