...
if __name__ == '__main__':
    ...
    Classifiers trained on Labeled(DS) or viceversa
    Classifiers tested on SS or viceversa
    '''
    ...
    # embedding_type = ['all', 'content_tags', 'DOM_RTED', 'VISUAL_PDiff']
    ...
    settings = ["train_on_SS", "train_on_DS"]
    # settings = ["train_on_DS"]
                ...
                for name, model in zip(names, classifiers):
                    ...
                    if name == "Threshold":
                        ...
                        if setting == "train_on_DS":
                            ...
                            # load SS as training set (all apps)
                            df_train = pd.read_csv('D:\\doc2vec\\dataset\\training_sets\\SS_threshold_set.csv')

                            # load Labeled(DS) as test set
                            df_test = pd.read_csv('D:\\doc2vec\\dataset\\training_sets\\DS_threshold_set.csv')
                        ...
                        if setting == "train_on_DS":
                            ...
                            # load SS as training set (all apps)
                            X_train = np.array(df_ss[feature]).reshape(-1, 1)
                            y_train = np.array(df_ss['HUMAN_CLASSIFICATION'])

                            # load Labeled(DS) as test set
                            X_test = np.array(df_labeled_ds[feature]).reshape(-1, 1)
                            y_test = np.array(df_labeled_ds['HUMAN_CLASSIFICATION'])
                    ...
                    d1 = pd.DataFrame(
                        {'Setting': setting,
                         'Model': ['DS_' + emb + '_' + vector_size[0] + 'epoch' + str(ep)],
                         'Embedding': [a],
                         'Classifier': [name],
                         'Accuracy': [accuracy],
                         'Precision': [precision],
