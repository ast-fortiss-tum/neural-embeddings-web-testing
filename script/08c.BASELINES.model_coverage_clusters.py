...
SETTINGS = ["within-apps"] # 'within-apps' or 'across-apps'
APPS = ['addressbook', 'claroline', 'dimeshift', 'mantisbt', 'mrbs', 'pagekit', 'petclinic', 'phoenix', 'ppma']

CLASSIFIERS = {

    'addressbook': [f"trained_classifiers/{SETTINGS[0]}-addressbook-svm-rbf-dom-rted.sav",
                    f"trained_classifiers/{SETTINGS[0]}-addressbook-svm-rbf-visual-pdiff.sav"],

    'claroline': [  f"trained_classifiers/{SETTINGS[0]}-claroline-svm-rbf-dom-rted.sav",
                    f"trained_classifiers/{SETTINGS[0]}-claroline-svm-rbf-visual-pdiff.sav"],

    'dimeshift': [  f"trained_classifiers/{SETTINGS[0]}-dimeshift-svm-rbf-dom-rted.sav",
                    f"trained_classifiers/{SETTINGS[0]}-dimeshift-svm-rbf-visual-pdiff.sav"],

    'mantisbt': [   f"trained_classifiers/{SETTINGS[0]}-mantisbt-svm-rbf-dom-rted.sav",
                    f"trained_classifiers/{SETTINGS[0]}-mantisbt-svm-rbf-visual-pdiff.sav"],

    'mrbs':     [    f"trained_classifiers/{SETTINGS[0]}-mrbs-svm-rbf-dom-rted.sav",
                    f"trained_classifiers/{SETTINGS[0]}-mrbs-svm-rbf-visual-pdiff.sav"],

    'pagekit': [    f"trained_classifiers/{SETTINGS[0]}-pagekit-svm-rbf-dom-rted.sav",
                    f"trained_classifiers/{SETTINGS[0]}-pagekit-svm-rbf-visual-pdiff.sav"],

    'petclinic': [  f"trained_classifiers/{SETTINGS[0]}-petclinic-svm-rbf-dom-rted.sav",
                    f"trained_classifiers/{SETTINGS[0]}-petclinic-svm-rbf-visual-pdiff.sav"],

    'phoenix': [    f"trained_classifiers/{SETTINGS[0]}-phoenix-svm-rbf-dom-rted.sav",
                    f"trained_classifiers/{SETTINGS[0]}-phoenix-svm-rbf-visual-pdiff.sav"],

    'ppma': [       f"trained_classifiers/{SETTINGS[0]}-ppma-svm-rbf-dom-rted.sav",
                    f"trained_classifiers/{SETTINGS[0]}-ppma-svm-rbf-visual-pdiff.sav"],
}

OUTPUT_CSV = False
filename = f'csv_results_table/rq3-{SETTINGS[0]}.csv'
...
if __name__ == '__main__':
    ...
    for app in APPS:
        ...
        for feature in np.arange(2):
            ...
            new_ss.to_csv(f'script/distance_matrices/SS_as_distance_matrix_{SETTINGS[0]}-{app}-{"rted" if feature==0 else "pdiff"}.csv') # Distance matrix with predictions

            # model coverage (rq2) starting here
            continue
            dictionary = {}
