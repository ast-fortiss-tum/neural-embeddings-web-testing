...
if __name__ == '__main__':
    ...
    for feature in ['content_tags', 'content', 'tags']:
        ...
        for app in APPS:
            print(f'{app} - {feature} - {setting}')
            ss = ss[ss['appname'] == app]
            cluster_file_name = f'{base_path}output/{app}.json' # file listing all clusters for the app and the corresponding states
            pred_file = f'{base_path}script/SS_as_distance_matrix_{app}.csv' # this includes only petclinic => TODO other files?
            predictions = pd.read_csv(pred_file, index_col=0) # symmetric matrix
                        ...
                        if model == []: # if model is empty, add the first state
                            ...
                            if bin_index == 0: # if the first state in the bin, i.e. the 'key' => compare with all existing states in the model
                                ...
                                for ms in model: # for each state already in the model
                                    ...
                                    if pred == NEAR_DUP_CLASS: # current state is clone/ND to a state in the model => do not add to model
                                        is_distinct = False
                                        break
                                ...
                                if pred == DISTINCT_CLASS: # distinct => add to the model, TODO: 0 CLONE?
